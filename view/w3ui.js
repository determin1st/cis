...
w3ui = function(){
  var DEP, CLONE, PARTIAL, PROXY, BOUNCE, THREAD, CORE, API;
  DEP = [[jQuery, "jQuery open-source library"], [Object.entries, "ECMAScript® 2018"], [window.requestAnimationFrame, "WHATWG HTML Living Standard"], [document.body.offsetLeft !== undefined, "CSSOM View Module 2016"]];
  DEP = function(){
    var b, i$, ref$, len$, a;
    b = true;
  ...
  PARTIAL = function(scope, func){
    ...
    a = arguments.length > 2
      ...
      : [];
    return function(){
      return func.apply(scope, a.concat(Array.apply(null, arguments)));
    };
  };
      ...
      try {
        ...
        a = (function(method){
          return function(obj, key, val){
            ...
            return method.apply(handler.scope$, arguments);
          };
        }.call(this, handler.set));
        b = (function(method){
          return function(obj, key){
            ...
            case "clone":
              return function(raw, merge){
                var a;
                a = merge
                  ? import$(CLONE(obj), merge)
                  : CLONE(obj);
                if (raw) {
                  return a;
                }
                return PROXY(a, handler);
              };
            }
            if (!method) {
              return obj[key];
        ...
        }.call(this, handler.get));
        c = clone$(handler);
        c.set = a;
        c.get = b;
        return new Proxy(obj, c);
      } catch (e$) {
        e = e$;
        a = e.fileName.replace(/^.*[\\\/]/, "");
        console.log("w3ui:[" + a + "::" + e.lineNumber + "]: " + e.message);
        return null;
      }
    }.call(this));
  };
  ...
  CORE = {
    ...
    sensor: {
      ...
      add: function(node){
        ...
        d.target[b] = node;
        d.sensor[b] = a;
        d.event[b] = PARTIAL(this, this.event, b);
        true;
      },
  ...
  API = function(selector){
    var node, style, i$, to$, a;
    if (!selector || typeof selector !== 'string') {
      return null;
    }
    if (!(node = $(selector)) || !node.length) {
      return null;
    }
    style = [];
    for (i$ = 0, to$ = node.length - 1; i$ <= to$; ++i$) {
      a = i$;
      style[a] = window.getComputedStyle(node[a]);
    }
    node.style = PROXY({
      index: 0
    }, {
      get: function(obj, k, prx){
        var a, b;
        if (typeof k !== 'string') {
          return null;
        }
        a = parseInt(k);
        if (!isNaN(a) && a >= 0 && k < node.length) {
          if (a === obj.index) {
            return prx;
          }
          b = clone$(prx);
      ...
      set: function(obj, k, v, prx){
        ...
        if (b === undefined) {
          ...
          node[obj.index].style.setProperty('--' + b, v);
        } else {
          node[obj.index].style[k] = v;
        }
        return true;
      }
    });
    node.box = PROXY({
      ...
      get: function(obj, k, prx){
        ...
        a = node.style[obj.index];
        b = node[obj.index];
        c = 0;
        switch (k) {
        case 'innerWidth':
          c = b.clientWidth - (a.paddingLeft + a.paddingRight);
          break;
        case 'innerHeight':
          c = b.clientHeight - (a.paddingTop + a.paddingBottom);
        }
        return c;
      }
    });
    node.textMeasure = function(text, fontSize, index){
      var me, a;
      ...
      if (!me.ctx) {
        ...
        me.ctx = a.getContext('2d');
      }
      if (!(a = node.style[index])) {
        return null;
      }
      if (!fontSize) {
        fontSize = a.fontSize;
      }
      a = [a.fontStyle, a.fontWeight, fontSize + 'px', a.fontFamily];
      me.ctx.font = a.join(' ');
      return me.ctx.measureText(text);
    };
    node.textMeasureFont = function(text, index){
      var a, b, c, d;
      index == null && (index = 0);
      if (!text || !typeof text === 'string') {
        return 0;
      }
      a = node.box[index].innerHeight;
      b = node.box[index].innerWidth;
      c = [0, a];
      while (c[1] - c[0] > 0.5) {
        if (!(d = node.textMeasure(text, a, index))) {
          return 0;
        }
        if (d.width <= b) {
          c[0] = a;
  ...
  API = PROXY(API, {
    set: function(ui, name, module){
      ...
      proto = clone$(CORE);
      proto.id = ui.uniqueId();
      proto.log = PARTIAL(proto, ui.log, name);
      if (!proto.inject(module, name)) {
        return false;
      }
      ui[0][name] = proto;
      module = function(){
        return CORE.api.create.apply(this, [ui, proto].concat(arguments[0]));
      };
      return jQuery.fn.extend((ref$ = {}, ref$[proto.name] = PROXY(module, {
        get: function(f, method, prx){
          var a, ref$, b;
          if (proto.api[method]) {
            return proto.api[method];
          }
          for (a in ref$ = proto.api.alias) {
            b = ref$[a];
            if (b.indexOf(method) >= 0) {
              return proto.api[a];
            }
          }
          return ui.log(method + "() unimplemented");
        },
        set: function(f, method, params, prx){
